...
fv.cfa <- run_cfa(sslong, inames = fv_items, fname = "FV",
                   waves = 1, missing = "pairwise", cluster = "SCHOOL_ID")
...
emp.cfa <- run_cfa(sslong, inames = emp_items, fname = "Lack_Empathy",
                   waves = 1:4, missing = "pairwise", cluster = "SCHOOL_ID")
...
sh.fits <- fits_wrapper(mod.list = list(sh.config$fit, sh.metric$fit,
                                         sh.strict$fit),
                         .id = "model") %>%
  mutate(model = c("Configural", "Metric/Scalar", "Strict"))

sh.mi <- bind_rows(compare_mods(sh.config$fit, sh.metric$fit),
                    compare_mods(sh.metric$fit, sh.strict$fit)) %>%
...
save(sh.mi.mod, sh.config, sh.metric, sh.strict, sh.fits, sh.mi,
...
# all.fs.temp <- bind_cols(factor.scores$Sexual_Harassment,
#                     factor.scores$Depression,
#                     factor.scores$Lack_of_Empathy,
#                     factor.scores$Delinquency,
#                     factor.scores$Substance_Use,
#                     factor.scores$School_Belonging)
